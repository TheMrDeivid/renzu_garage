...
Config.target = false -- use ox_target,qb-target with peds when interacting to Garage.
--GENERAL SETTING
...
Config.UniqueCarperGarage = true -- if false, show all vehicles to all garage location! else if true, Vehicles Saved in Garage A cannot be take out from Garage B for example.
Config.GarageKeys = false -- Enable Player to Give Public Garage Keys to enable vehicle sharing
-- BLIPS --
Config.BlipNamesStatic = false -- if true no more garage a garage b blip names from MAP , only says  Garage
--GENERAL SETTING
...
Config.Ox_Inventory = false -- ox_inventory for vehicle keys as item https://github.com/overextended/ox_inventory if this is enable, vehiclekeys command will be disable as vehicle sharing is now item based
...
Config.EnableImpound = false -- enable/disable impound
Config.EnableHeliGarage = false -- enable/disable Helis
Config.PlateSpace = false -- enable / disable plate spaces (compatibility with esx 1.1?)
...
Config.PropertyQuickPick = false -- quickpick -- deprecated method 1.72
...
Config.LockAllLocalVehicle = false -- lock all vehicles in area if not unlocked state (This are not looped, its being locked only when you Press F) (if you are using F Keybind in other script, this might not work properly) ex. in renzu_hud, F is being used, you may need to disable it there.
Config.LockParkedLocalVehiclesOnly = false -- if this is true and Config.LockAllLocalVehicle is true, its useless, this is useful if Config.LockAllLocalVehicle is false and this is true
Config.EnableLockpick = false -- enable lock pick item and command function
Config.EnableDuplicateKeys = false -- Carlock Purpose and to bypass hotwired
Config.GiveKeystoMissionEntity = false -- important if you dont want to hotwire mission vehicle, eg. trucker, taxi, delivery vehicles. (this will give car lock too)
Config.RequestDuplicateCoord = {
    [1] = vec3(218.74,-811.37,30.65), --x , y, z -- where the player can request duplicate keys
}
Config.RemoveKeys = false -- remove keys upon storing vehicle
--Hotwire
Config.EnableHotwire = false -- enable hotwire if keys is not present in state
Config.HotwireLevel = 5 -- how many pins to complete (renzu_lockgame) https://github.com/renzuzu/renzu_lockgame
-- Lockpick
Config.EnableLockpickCommand = false -- enable / disable command
Config.LockpickCommand = 'lockpick1' -- command name
Config.LockpickItem = 'lockpick1' -- itemname -- makesure you register this
Config.LockpickLevel = 3 -- how many pins to complete
-- Fail Alert
Config.EnableAlert = false
...
-- Carlock
Config.CarlockKey = '' -- Keyboard (changable in keybinds FIVEM setting)
